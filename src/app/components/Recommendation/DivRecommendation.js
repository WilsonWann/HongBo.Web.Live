...
export const DivRecommendation = () => {
  ...
  const [selectedStreamRoom, setSelectedStreamRoom] = useState(
    isListExist(streamRoomList) && streamRoomList[0]
  );
  const [selectedRoomID, setSelectedRoomID] = useState(
    isListExist(streamRoomList) && streamRoomList[0].RoomID
  );
  useEffect(() => {
    if (isListExist(streamRoomList) && selectedRoomID) {
      setSelectedStreamRoom(
        streamRoomList.find(
          (streamRoom) => streamRoom.RoomID === selectedRoomID
        )
      );
    }
    if (isListExist(streamRoomList) && !selectedRoomID) {
...
const StyledDiv = styled.div`
  &:before {
    ...
    "/assets/images/Android/TITLE_1.png"});
    position: absolute;
    width: 395px;
    height: 84px;
    top: -88px;
  ...
  "/assets/images/Android/1_BG.png"});
  background-repeat: no-repeat;
  background-size: contain;
